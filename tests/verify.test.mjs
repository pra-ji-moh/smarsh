...
test('the rational domain is respected, and that is not a bug', () => {
  // `i > 0` does not give `i - 1 >= 0` unless i is an integer, and Sarvm's `num`
...
test('verify and prove agree on the shipped contracts example', () => {
  const source = fs.readFileSync(path.join(ROOT, 'examples', 'contracts.sarvm'), 'utf8');
  const verified = verifyProgram(parse(source, 'contracts.sarvm'));
