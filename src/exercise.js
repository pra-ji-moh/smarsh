...
//
// Lives in its own module because two callers need it: `sarvm prove`, and the
...
export function genArg(rng, trial, index) {
  ...
  if (r < 0.85) return ['', 'a', 'hello', 'Sarvm', 'x y z'][Math.floor(rng.next() * 5)];
...
// Sarvm is dynamically typed and contracts cannot state types, so a generated
