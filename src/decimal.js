...
export class Decimal {
  ...
  get smarshType() { return 'dec'; }

  static parse(text, line = null) {
    const s = String(text).trim();
    const m = /^([+-]?)(\d*)(?:\.(\d*))?(?:[eE]([+-]?\d+))?$/.exec(s);
    if (!m || (m[2] === '' && (m[3] === undefined || m[3] === ''))) {
      throw smarshError('ValueError', `\`${s}\` is not a decimal number`, line)
  ...
  static fromInteger(n, line = null) {
    ...
    if (!Number.isSafeInteger(n)) {
      throw smarshError('ValueError', `\`${n}\` is beyond what a num holds exactly`, line);
    }
  ...
  div(other, scale, line = null) {
    if (other.c === 0n) throw smarshError('ZeroDivisionError', 'division by zero', line);
    const target = scale ?? Math.max(this.scale, other.scale, 2);
    if (target > MAX_SCALE) {
      throw smarshError('ValueError', `a scale of ${target} is beyond the maximum of ${MAX_SCALE}`, line);
    }
...
export function expectDec(v, what, line) {
  ...
  if (!(u instanceof Decimal)) {
    throw smarshError('TypeError', `${what} must be a dec`, line)
...
export function decimalBinary(op, l, r, line) {
  ...
  if (!left || !right) {
    ...
    throw smarshError('TypeError', `\`${op}\` is not defined between \`dec\` and this value`, line);
  }
  ...
  switch (op) {
    ...
    case '>=': return left.compare(right) >= 0;
    default:
      throw smarshError('TypeError', `\`${op}\` is not defined on \`dec\``, line)
