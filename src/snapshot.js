...
import {
  ContextWindow, Ledger, Tainted, SmarshFunction, NativeFunction, unwrap, countTokens,
} from './values.js';
...
function encode(value, skipped, path) {
  ...
  if (v instanceof SmarshFunction || v instanceof NativeFunction || v instanceof AgentTemplate) {
    return null;
  }

  skipped.push(`${path} (${v && v.smarshType ? v.smarshType : typeof v})`);
  return null;
}
...
function decode(node, agentsById, line) {
  ...
  switch (node.t) {
    ...
    case 'agentref': {
      ...
      if (!ref) throw smarshError('RestoreError', `the snapshot mentions agent #${node.id}, which is not in it`, line);
      return ref;
    }
    default:
      throw smarshError('RestoreError', `unknown value kind '${node.t}' in the snapshot`, line);
  }
}
...
export function restore(interp, state, line = null) {
  if (!state || state.version !== VERSION) {
    throw smarshError('RestoreError',
