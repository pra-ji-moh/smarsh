...
Sarvm follows [Semantic Versioning](https://semver.org/spec/v2.0.0.html), with
...
- **`sarvm build` output** — a bundle produced by version *N* keeps working on
...
- **Anything under `src/`.** The JavaScript API is internal. Import
  `bin/sarvm.mjs` as a program, not `src/interpreter.js` as a library.
...
Sarvm supports the Node versions in CI: currently **18, 20 and 22**. Dropping one
